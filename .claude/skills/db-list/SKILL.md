...
| `password` | string | нет | Пароль (plaintext — НЕ рекомендуется, используй `passwordEnv`) |
| `passwordEnv` | string | нет | Имя переменной окружения с паролем. Имеет приоритет над `password`. Скриптам передаётся как `-PasswordEnv <имя>` — пароль не попадает в командную строку и логи |
...
> **Безопасность**: если у базы задано `passwordEnv` — НЕ подставляй пароль в команду. Скриптам навыков передавай `-PasswordEnv <имя переменной>` вместо `-Password`; скрипт сам разрешит значение (process env → user/machine реестр). Поле `password` оставлено для обратной совместимости.
